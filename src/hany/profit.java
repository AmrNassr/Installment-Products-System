         ...
         || dayy == 5 || dayy == 6 || dayy == 7 || dayy == 8 || dayy == 9 )
 {
    ...
    String to=year2.getText()+"-"+month2.getText()+"-"+day2.getText();
        try {
             ...
             String sql="SELECT sum(total) as x  FROM hany.profit WHERE date BETWEEN  CAST('"+from+"' AS DATE) AND  CAST('"+to+"' AS DATE) order by date;";
             pst=(PreparedStatement) DBConnect.con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
                 rab77 = rs.getString("x");
    ...
    try {
             ...
             String sql="SELECT sum(gomla) as x  FROM hany.profit WHERE date BETWEEN  CAST('"+from+"' AS DATE) AND  CAST('"+to+"' AS DATE) order by date;";
             pst=(PreparedStatement) DBConnect.con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
                 gomlaa = rs.getString("x");
     ...
     try {
    String sql="SELECT sum(price) as x  FROM hany.profit WHERE date BETWEEN  CAST('"+from+"' AS DATE) AND  CAST('"+to+"' AS DATE) order by date;";
             pst=(PreparedStatement) DBConnect.con.prepareStatement(sql);
             rs=pst.executeQuery();
             while(rs.next()){
                 pricee = rs.getString("x");
     ...
     InputStream in=null;
      try {
          ...
          in = this.getClass().getResourceAsStream("/reports/profit.jasper");
          String reportpath= "\\\\192.168.1.15\\sadam\\profit.jasper";
    Map<String,Object> para = new HashMap<String,Object>();
            para.put("from",from );
            para.put("to",to );
             para.put("rab7",rab77 );
            para.put("price",pricee );
             para.put("gomla",gomlaa );
