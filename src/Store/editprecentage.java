 ...
 * @author amnassar
 */
public class editprecentage extends javax.swing.JFrame {
    ...
    public editprecentage() throws ParseException {
        initComponents();
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel17, new org.netbeans.lib.awtextra.AbsoluteConstraints(310, 80, 120, 30));

        name.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        jPanel1.add(name, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 80, 80, 40));
        ...
        jPanel1.add(save, new org.netbeans.lib.awtextra.AbsoluteConstraints(180, 160, 120, 40));
    ...
    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        ...
        try {
            int precentage = Integer.parseInt(name.getText());

            if (name.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, " من فضلك ادخل النسبة");
            } else {
                int dialogButton = JOptionPane.YES_NO_OPTION;
                int dialogResult = JOptionPane.showConfirmDialog(this, "هل تريد حفظ العمليه", "تاكيد العمليه", dialogButton);
                if (dialogResult == 0) {
                    try {
                        /* String sql = "UPDATE hany.storeitemlist SET precentage = '"+precentage+"' , total = (price *)";
                         pst = (PreparedStatement)DBConnect.con.prepareStatement(sql);
                         pst.execute();
                         JOptionPane.showMessageDialog(null, "تم تعديل النسبة بنجاح");
                         name.setText("");*/
                        String sqll = "SELECT * FROM hany.storeitemlist ";
                        pst = (PreparedStatement) DBConnect.con.prepareStatement(sqll);
                        rs = pst.executeQuery();
                        while (rs.next()) {
                            float pricee = Float.parseFloat(rs.getString("price"));
                            String namee= rs.getString("name");
                          String  sqlll = "UPDATE hany.storeitemlist SET precentage = '"+precentage+"' , total = ("+pricee+" * (1+("+precentage+"/100))) where name = '"+namee+"'";
                         pst = (PreparedStatement)DBConnect.con.prepareStatement(sqlll);
                         pst.execute();
                        }
                        JOptionPane.showMessageDialog(null, "تم تعديل النسبة بنجاح");
                    } catch (Exception e) {
